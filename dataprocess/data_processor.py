...
# Driver code
class ERTESDDataProcessor(object):
    def __init__(self,
                 root,
                 tokenizer,
                 is_lower=False,
        ...
        if "ccl" in dataset_name:
            self.pred2text=model.rel2text.ccl_rel2text
            cnt = 1
            ...
            self.pred2text=model.rel2text.scierc_rel2text
            cnt = 1
            for k in self.pred2text:
                v = self.pred2text[k]
                if isinstance(v, int):
            ...
            self.pred2text=model.rel2text.conll_rel2text
            cnt = 1
            for k in self.pred2text:
                v = self.pred2text[k]
                if isinstance(v, int):
    ...
    def _pre_process(self, path, token_len, is_predict, data_nums):
        ...
        for line in tqdm(data):
            if len(line["relation_list"]) == 0:
                continue
            text = line["text"]
            # print(self.tokenizer.encode('你好[unused1]'))
