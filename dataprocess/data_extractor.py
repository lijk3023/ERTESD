...
def ERTESD_extractor(tokenizer,
                   dataset,
                   predictions,
                   path,
                   ):
...
def get_uncertain_triple(head_pred, tail_pred, span_pred, dataset, tokenizer, token_len, idx2pred, path):
    uncertain_data = []
    curr_data_idx = 0
    for head_pred, tail_pred, span_pred in zip(head_pred, tail_pred, span_pred):
        input_ids = dataset[curr_data_idx]["input_ids"]
        text = dataset.texts[curr_data_idx]
        uncertain_spo_text = set()
        ...
        for l, r in e2e:
            ...
            for l_span in l_spans:
                for r_span in r_spans:
                    ...
                    uncertain_elements = []
                    if head_pred[l_s][r] == 2 and (head_pred[l_s][token_len:] == 1).any():
                        uncertain_elements.append("subject")
                    if tail_pred[r][l_s] == 2 and (tail_pred[:token_len][r_s] == 1).any():
                        uncertain_elements.append("object")
                    if any(head_pred[l_s][r] == 2 for r in range(token_len, token_len + len(idx2pred))) or any(
                            tail_pred[r][r_s] == 2 for r in range(token_len, token_len + len(idx2pred))):
                        uncertain_elements.append("relation")
        ...
        uncertain_data.append({
            "text": text,
            "uncertain_spo_list": list(uncertain_spo_text),
        })
        curr_data_idx += 1
    with open(path, "w") as wp:
        json.dump(uncertain_data, wp, indent=2, ensure_ascii=False)
        
def ERTESD_span_extractor(tokenizer,
                   dataset,
                   predictions,
                   path,
                   reevmode = False
    ...
    idx2pred = dataset.data_processor.idx2pred
    extract_data = []
    path3 =  os.path.join(path, dataset.mode + '_unc_sard2.json')
    path2 = os.path.join(path, dataset.mode + '_unc_sard.json')
    path = os.path.join(path, dataset.mode + '_predict_sard.json')
    head_preds, tail_preds, span_preds, head_unc, tail_unc, span_unc = predictions.predictions
    ...
    with open(path, "w") as wp:
        json.dump(extract_data, wp, indent=2, ensure_ascii=False)
    if reevmode:
        get_uncertain_triple(head_unc, tail_unc, span_unc, dataset, tokenizer, token_len, idx2pred, path2)
    return all_metirc_results["all-prec"], all_metirc_results["all-recall"], all_metirc_results["all-f1"]
